...
mints an `agent_id`; that is yours. Pass it
on every `override_accept_case`.
...
Be lenient when the source errored and Snowflake succeeded — still use
...
from the prompt. One call per accepted hash, same minted `agent_id`:
