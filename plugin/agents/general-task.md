...
```
configure(project_dir="<projectDir>", agent_id="<agentId>")
transition_status(status="begin", where="id = '<objectId>'", agent_id="<agentId>")
```
...
```
migration_status(mode="next_task", object_ids=["<objectId>"], agent_id="<agentId>")
```
...
When the executor is `update_registry`, add `agent_id="<agentId>"` yourself.
...
Then call `migration_status(mode="next_task", object_ids=["<objectId>"], agent_id="<agentId>")`
...
arriving on a different payload. Do not run it. Call `next_task` with your
`agent_id` so a job-backed block can carry `wait`.
...
transition_status(status="note", task="<the task you were on>",
                  ...
                  where="id = '<objectId>'", agent_id="<agentId>")
```
...
Do not pass your `agentId`. Do not `begin` for that child. Wait. Then
...
the same failures, and the server refuses it under your `agentId` and
refuses an omitted `agent_id`. Spawn **one** foreground
...
Do not pass your `agentId`. Do not tell it the verdict or hand it a
...
the first verifier rejected. `override_accept_case` under your walker id
is still refused; the new child uses its own minted id.
...
read more source to settle it. Read more. And do not escalate what already has a
route: a SQL bug is `outcome="failed", error="sql"` and goes to the fix loop, a
flaky environment is `error="infra"`. A missing dependency is not a stamp —
return `waiting` / `partial` and let the walk derive `blockedOn`.
...
```
transition_status(status="finish", where="id = '<objectId>'", agent_id="<agentId>")
```
