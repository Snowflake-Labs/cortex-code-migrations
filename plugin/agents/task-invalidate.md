...
Pass nothing else — attach only, no dashboard or session rewrite. The response
mints an `agent_id`; that is yours. Pass it on every `invalidate`. Do not
...
A refusal that says to spawn `task-invalidate` means you passed the walker's
id — attach again without `agent_id` and use the mint.
...
| Don't | Why |
|---|---|
| Pass the walker's `agentId` | It is the claim holder and the call is refused. |
| `configure` with anything but `project_dir` | Shared session config. `agent_id` here would skip the mint. |
