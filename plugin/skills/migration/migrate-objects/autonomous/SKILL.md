...
agentId:            <the `agentId` minted with that object in 2b, or with its `answered` entry in 2e>
...
- If no live object-walker remains **and** 2b returned nothing (or
  ...
  cursor=<last>)` instead of ending the turn. Without `confirm=true` the
...
That is a later send: `task(resume=<that object's stored resume id>)` with
...
transition_status(status="answer", where="id IN (...)", task="<task>",
                  ...
                  reason="<the user's words>", agent_id="0000")
```
