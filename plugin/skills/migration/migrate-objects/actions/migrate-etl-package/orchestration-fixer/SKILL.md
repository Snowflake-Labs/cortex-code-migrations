...
description: "Fix orchestration elements with EWI issues using test-file workbench. Scoped to elements assigned to the current phase. Use when the migrate-etl-package skill invokes fixing for an orchestration phase."
---
...
- `ROADMAP.md` — phase plan with element assignments (authored by migrate-etl-package)
- The orchestration `.sql` file path (from scan results)
- Test files at `<PACKAGE>/.migrate-etl-package/tests/orchestration/<task_procedure_name>/<element_name>.sql` (generated by orchestration-test-gen)
- `<PACKAGE>/.migrate-etl-package/tests/orchestration/test_report.md` with baseline results
...
Ensure a backup exists before editing. The parent skill creates `<PACKAGE>/.migrate-etl-package/original/` during setup; create it if missing.
...
4. **Check prior fixes (cross-learning)** — search `{PACKAGE}/.migrate-etl-package/artifacts/tracking/fix_log.md` for entries matching `**EWI/FDM**: <ISSUE_CODE>` (all entries are successful fixes — the file only records patterns that worked). If prior fixes exist for the same issue code:
...
7. **Test the fix using the test file** — the test file is your workbench:
   ...
   a. **Find the test file**: `<PACKAGE>/.migrate-etl-package/tests/orchestration/<task_procedure_name>/<element_name>.sql`
...
10. **Append to fix log** (Main Mode only — Task Mode agents write to `learnings_batch_{B}.md` instead; the orchestrator merges learnings into fix_log.md after all agents complete) — write a record to `{PACKAGE}/.migrate-etl-package/artifacts/tracking/fix_log.md`:
   ```markdown
   ## Fix Record
...
2. Present the summary to the user:
   ```
   Phase <N> fixing complete for <package_name>:
     Fixed: N elements
...
3. Return to the parent skill (migrate-etl-package/SKILL.md) for next steps.

### Troubleshooting

#### Test file not found for element
- Verify test-gen ran for this phase: check `<PACKAGE>/.migrate-etl-package/tests/orchestration/<task_procedure_name>/` for `.sql` files
...
- Check `{PACKAGE}/.migrate-etl-package/artifacts/tracking/fix_log.md` for prior attempts — review what was tried
...
- Fix log path: `PACKAGE/.migrate-etl-package/artifacts/tracking/fix_log.md` (READ-ONLY — consult if exists; absent in Phase 1)
...
- Test files (ACT updates only): `PACKAGE/.migrate-etl-package/tests/orchestration/<task_procedure_name>/`
