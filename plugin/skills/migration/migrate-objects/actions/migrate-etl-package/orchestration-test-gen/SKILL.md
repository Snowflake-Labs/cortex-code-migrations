...
description: "Generate AAA orchestration tests (isolated or grouped) for ETL-to-Snowflake elements. Scoped to elements assigned to the current phase in ROADMAP.md. Use when the migrate-etl-package skill invokes test generation for an orchestration phase."
...
- `session_status.json` — with `source_file_path` set, pending orchestration elements, and `package_path`
...
- `ROADMAP.md` — defines phases, element assignments, and test strategies (authored by migrate-etl-package)
...
```
<PACKAGE>/.migrate-etl-package/tests/orchestration/
  <task_procedure_name>/
...
- Isolated: `<PACKAGE>/.migrate-etl-package/tests/orchestration/<task_procedure_name>/<element_name>.sql`
- Grouped: `<PACKAGE>/.migrate-etl-package/tests/orchestration/<task_procedure_name>/grouped_<group_name>.sql`
...
1. Read `session_status.json` for the `elements` list and statuses; read `scan_results.json` for the `statements` array (each `CREATE TASK`/`CREATE PROCEDURE` and its child elements, delimited by `---- Start`/`---- End` tags)
...
2. **Test bug (wrong mock/assertion)** → fix in `.migrate-etl-package/tests/orchestration/`, retry up to 3 times
...
4. Write `<PACKAGE>/.migrate-etl-package/tests/orchestration/test_report.md` with:
...
6. Return to parent skill (migrate-etl-package/SKILL.md) for orchestration fixing.
...
- Test files: `PACKAGE/.migrate-etl-package/tests/orchestration/<task_procedure_name>/`
...
2. **Generate test file**:
   ...
   - Write test files to: `PACKAGE/.migrate-etl-package/tests/orchestration/<task_procedure_name>/`
