...
```
{PACKAGE}/.migrate-etl-package/artifacts/phases/phase_{N}/_infrastructure.sql
```
...
Copy full DDL from existing test artifacts — search for `InitVariablesFromConfig` in `.migrate-etl-package/tests/` and paste verbatim, replacing the schema qualifier with `{SCHEMA}`.
...
Copy full DDL from existing test artifacts — search for `BuildDbtVarsJsonUDF` in `.migrate-etl-package/tests/` and paste verbatim, replacing the schema qualifier with `{SCHEMA}`.
