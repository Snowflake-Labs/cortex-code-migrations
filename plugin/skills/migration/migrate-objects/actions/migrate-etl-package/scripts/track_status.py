...
Essential commands (always used):
    python track_status.py init <scan_results.json>
...
def cmd_init(scan_path: Path) -> None:
    ...
    source_file_path = scan.get("source_file_path") or scan.get("dtsx_path")
    ...
    session = {
        ...
        "package_path": scan["package_path"],
        "orchestration_file": orch_file,
        "orchestration_file_hash": orch_hash,
        "source_file_path": source_file_path,
        "dtsx_path": source_file_path,  # deprecated alias for backward compatibility
...
def _validate_orch_artifacts(session: dict, phase_num: int, pkg_path: str) -> list[str]:
    ...
    if pkg_path:
        phase_dir = Path(pkg_path) / ".migrate-etl-package" / "artifacts" / "phases" / f"phase_{phase_num}"
        # Search recursively: artifacts may be in task subdirectories
        if not any(phase_dir.rglob("baseline_batch_*.md")):
            errors.append(f"Missing artifact: baseline_batch_*.md not found under {phase_dir}")
        if not any(phase_dir.rglob("batch_*.md")):
            errors.append(f"Missing artifact: batch_*.md not found under {phase_dir}")
        if not any(phase_dir.rglob("learnings_batch_*.md")):
            errors.append(f"Missing artifact: learnings_batch_*.md not found under {phase_dir}")
        if not any(phase_dir.rglob("apply_report.md")):
            errors.append(f"Missing artifact: apply_report.md not found under {phase_dir}")
    else:
        errors.append("Missing artifact: package_path not set in session; cannot locate phase artifacts")
...
def _validate_dbt_artifacts(session: dict, phase_num: int, pkg_path: str) -> list[str]:
    ...
    - .migrate-etl-package/tests/dbt/{PROJECT}/seeds/ has at least 1 .csv
    - .migrate-etl-package/tests/dbt/{PROJECT}/tests/ has at least 1 .sql
    - .migrate-etl-package/tests/dbt/{PROJECT}/test_report.md exists
    ...
    for project in sorted(projects):
        if pkg_path:
            mep = Path(pkg_path) / ".migrate-etl-package"
            seeds_dir = mep / "tests" / "dbt" / project / "seeds"
            tests_dir = mep / "tests" / "dbt" / project / "tests"
            report_file = mep / "tests" / "dbt" / project / "test_report.md"
            phase_dir = mep / "artifacts" / "phases" / f"phase_{phase_num}"
            learnings_file = phase_dir / f"dbt_learnings_{project}.md"

            if not seeds_dir.is_dir() or not any(seeds_dir.glob("*.csv")):
                errors.append(f"Missing dbt artifact: {seeds_dir} has no .csv seed files")
            if not tests_dir.is_dir() or not any(tests_dir.glob("*.sql")):
                errors.append(f"Missing dbt artifact: {tests_dir} has no .sql test files")
            ...
            errors.append("Missing artifact: package_path not set in session; cannot locate dbt test artifacts")
...
def _validate_phase_artifacts(session: dict, phase_num: int) -> list[str]:
    ...
    if scope in ("final-validation", "validation"):
        ...
        if pkg_path:
            report = Path(pkg_path) / ".migrate-etl-package" / "artifacts" / "report.html"
            if not report.is_file():
                errors.append(f"Missing artifact: {report} not found")
        else:
            errors.append("Missing artifact: package_path not set; cannot locate report.html")
        return errors
...
def cmd_validate_phase(status_path: Path, phase_num: int, list_test_files: bool = False) -> None:
    ...
    if list_test_files:
        pkg_path = session.get("package_path", "")
        test_dir = Path(pkg_path) / ".migrate-etl-package" / "artifacts" / "phases" / f"phase_{phase_num}" if pkg_path else None
        seen: set[str] = set()
        found: list[str] = []
        for el in phase_els:
...
def main() -> None:
    ...
    if command == "init":
        if len(sys.argv) < 3:
            print("Usage: python track_status.py init <scan_results.json>", file=sys.stderr)
            sys.exit(1)
