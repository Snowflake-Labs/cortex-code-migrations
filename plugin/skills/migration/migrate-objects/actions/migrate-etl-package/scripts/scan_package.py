#!/usr/bin/env python3
"""Scan a SnowConvert ETL package folder and produce a structural JSON index.

Usage:
    python scan_package.py <package_folder> [<source_file>] [--platform <platform_id>]
...
Output:
    <package_folder>/.migrate-etl-package/artifacts/planning/scan_results.json
"""
...
def find_orchestration_file(pkg_path: Path) -> Path | None:
    """Find the main orchestration SQL file in the package root."""
    stem_match = pkg_path / f"{pkg_path.name}.sql"
    if stem_match.is_file():
        return stem_match
    sql_files = [f for f in pkg_path.iterdir() if f.suffix == ".sql" and f.is_file()]
    if len(sql_files) == 1:
        return sql_files[0]
    return None
...
def find_dbt_projects(pkg_path: Path) -> list[dict]:
...
def find_reports_dir(pkg_path: Path) -> Path | None:
    """Walk upward from package folder looking for Reports/SnowConvert/."""
    current = pkg_path.resolve()
...
def load_assessment_csv(reports_dir: Path, pkg_name: str) -> dict[str, dict] | None:
...
def print_summary(result: dict) -> None:
    ...
    print(f"\n{'=' * 60}")
    print(f"  ETL Package Scan: {result['package_name']}")
    print(f"{'=' * 60}")
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python scan_package.py <package_folder> [<source_file>] [--platform <platform_id>]", file=sys.stderr)
        sys.exit(1)
    ...
    orch_file = find_orchestration_file(pkg_path)
    dbt_projects = find_dbt_projects(pkg_path)

    if not orch_file and not dbt_projects:
        print(f"Error: no orchestration SQL and no dbt projects found in '{pkg_path}'", file=sys.stderr)
        sys.exit(1)

    reports_dir = find_reports_dir(pkg_path)
    ...
    assessment = None
    if reports_dir:
        assessment = load_assessment_csv(reports_dir, pkg_name)
    ...
    result = {
        ...
        "dtsx_path": source_file_str,  # deprecated alias for backward compatibility
    ...
    out_dir = pkg_path / ".migrate-etl-package" / "artifacts" / "planning"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_file = out_dir / "scan_results.json"
