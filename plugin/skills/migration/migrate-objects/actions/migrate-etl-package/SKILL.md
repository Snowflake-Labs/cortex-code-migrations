...
Spawn context-mapping agents in parallel. The orchestration mapper always runs. The dbt mapper runs only if `scan_results.json` contains dbt_projects.

**Orchestration context mapper** (always):
Use the `task` tool to spawn a `general-purpose` agent with `name="context-mapper"`, `run_in_background=true`, and the prompt template at `{SKILL_DIR}/reference/agent-prompts/context-mapper.md` (substitute `{ORCH_SQL_PATH}`, `{SOURCE_FILE_PATH}`, `{PACKAGE}`, `{SKILL_DIR}`, `{PLATFORM_DIR}` placeholders).
...
Use the `task` tool to spawn a `general-purpose` agent with `name="dbt-context-mapper"`, `run_in_background=true`, and the prompt template at `{SKILL_DIR}/reference/agent-prompts/dbt-context-mapper.md` (substitute `{PACKAGE}`, `{SOURCE_FILE_PATH}`, `{TRANSFORMATION_GUIDE}` placeholders).
...
> **No team needed** — parallel standalone agents during planning. Use the `team_create` tool to create per-phase teams during Execution only.
...
### Step 1: Read State
Read `{PACKAGE}/.migrate-etl-package/artifacts/tracking/STATE.md` → identify current phase number and status.
Read `{PACKAGE}/.migrate-etl-package/artifacts/planning/ROADMAP.md` → find the next pending phase (first phase without `[x]` in Phase Overview).

### Step 2: Execute Phase
Read the current phase's **Execution Steps** section in the ROADMAP.
Find the first unchecked `- [ ]` step — that is the next step to execute.

**Ensure cortex tracking exists:**
Run `cortex ctx show tasks` via bash to check for an active task for this phase.
- If no task exists (fresh start or session restart): run Step {P}.0's bash block
  to create the cortex task and steps.
  If some steps are already `[x]` in the ROADMAP, mark the corresponding
  cortex steps done immediately so cortex matches the ROADMAP state.
- If a task already exists: continue from the first unchecked step.

**Execute each step in order:**
1. Read the step's full instructions in the ROADMAP
2. Execute them
3. Mark `[x]` next to the step in the ROADMAP
4. Mark the corresponding cortex step done (`cortex ctx step done <id>` —
   find the ID via `cortex ctx show tasks`)

When all steps are `[x]`, the phase is complete.
The last step directs you to the next phase's Step 0.
