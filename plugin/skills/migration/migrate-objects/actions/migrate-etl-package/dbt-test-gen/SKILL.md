...
description: >
  ...
  the migrate-etl-package skill invokes dbt test generation.
license: Proprietary. See License-Skills for complete terms
---

# dbt Test Generator

Generate and run semantic test cases for dbt project models based on the original dataflow logic in the source definition file. Tests are generated BEFORE fixes are applied, serving as an independent oracle derived from the source-of-truth (the source definition file). All generated test artifacts are stored in `<PACKAGE>/.migrate-etl-package/tests/dbt/` to keep the original dbt project clean.
...
Read ROADMAP.md for current phase (authored by migrate-etl-package with package-specific reasoning). Process only the dbt project(s) assigned to this phase.

## Input

This sub-skill expects:
- `session_status.json` -- with `source_file_path` set, `package_path`, and dbt projects in status `pending` or `dbt-tested`
...
All generated test artifacts go into the `.migrate-etl-package` folder, NOT into the dbt project itself:

```
<PACKAGE>/.migrate-etl-package/tests/dbt/<project_name>/
  seeds/
  ...
  tests/
    ...
    ...
  migrate_etl_package_test_schema.yml
  migrate_etl_package_test_sources.yml
...
Create seed files under `<PACKAGE>/.migrate-etl-package/tests/dbt/<project_name>/seeds/`. Header: source column names exactly. Formatting: dates `YYYY-MM-DD`, timestamps `YYYY-MM-DD HH:MI:SS`, booleans `true`/`false`.

### Step 2.3: Configure Source Overrides

Create `<PACKAGE>/.migrate-etl-package/tests/dbt/<project_name>/migrate_etl_package_test_sources.yml` to re-point source references to the seed tables:
...
Create `<PACKAGE>/.migrate-etl-package/tests/dbt/<project_name>/dbt_project_patch.yml`:

```yaml
seeds:
  <project_name>:
    migrate_etl_package_tests:
      +schema: "{{ target.schema }}"
...
All test files go to `<PACKAGE>/.migrate-etl-package/tests/dbt/<project_name>/tests/`. Generate tests for **every model** — staging, intermediate, AND mart.

### Step 3.1: Schema Tests

Create `<PACKAGE>/.migrate-etl-package/tests/dbt/<project_name>/migrate_etl_package_test_schema.yml` with schema-level tests.
...
Create `.sql` test files under `<PACKAGE>/.migrate-etl-package/tests/dbt/<project_name>/tests/`. Each test follows the AAA pattern with clearly labeled sections and dbt convention (returns rows that FAIL; 0 rows = pass).

**Template -- every singular test must follow this structure:**

```sql
-- tests/assert_<model_name>_<what_is_tested>.sql
{{ config(tags=["migrate_etl_package_test"]) }}
...
Copy the generated files from `.migrate-etl-package` into the dbt project:

```
<PACKAGE>/.migrate-etl-package/tests/dbt/<project_name>/seeds/*    -> <DBT_PROJECT>/seeds/migrate_etl_package_tests/
<PACKAGE>/.migrate-etl-package/tests/dbt/<project_name>/tests/*    -> <DBT_PROJECT>/tests/migrate_etl_package/
<PACKAGE>/.migrate-etl-package/tests/dbt/<project_name>/migrate_etl_package_test_sources.yml -> <DBT_PROJECT>/models/migrate_etl_package_test_sources.yml
<PACKAGE>/.migrate-etl-package/tests/dbt/<project_name>/migrate_etl_package_test_schema.yml -> <DBT_PROJECT>/models/migrate_etl_package_test_schema.yml
```
...
```bash
dbt seed --select tag:migrate_etl_package_test --project-dir <DBT_PROJECT_PATH> --profiles-dir ~/.dbt
```

### Step 4.3: Build the Models (Act)

```bash
dbt run --select tag:migrate_etl_package_test --project-dir <DBT_PROJECT_PATH> --profiles-dir ~/.dbt
```

### Step 4.4: Run the Tests (Assert)

```bash
dbt test --select tag:migrate_etl_package_test --project-dir <DBT_PROJECT_PATH> --profiles-dir ~/.dbt
```
...
2. Present the summary to the user:
   ```
   dbt test generation complete for <package_name>:
   ...
   Test artifacts: <PACKAGE>/.migrate-etl-package/tests/dbt/<project_name>/
   ```

3. Write detailed test report to `<PACKAGE>/.migrate-etl-package/tests/dbt/<project_name>/test_report.md` including:
...
4. **MANDATORY self-check before returning** -- verify all artifacts exist by checking each path:
   ```
   {PACKAGE}/.migrate-etl-package/tests/dbt/{PROJECT_NAME}/seeds/       -- must contain at least 1 .csv file
   {PACKAGE}/.migrate-etl-package/tests/dbt/{PROJECT_NAME}/tests/        -- must contain at least 1 .sql file
   {PACKAGE}/.migrate-etl-package/tests/dbt/{PROJECT_NAME}/migrate_etl_package_test_schema.yml   -- must exist
   {PACKAGE}/.migrate-etl-package/tests/dbt/{PROJECT_NAME}/migrate_etl_package_test_sources.yml  -- must exist
   {PACKAGE}/.migrate-etl-package/tests/dbt/{PROJECT_NAME}/test_report.md              -- must exist
   ```
...
5. Return to the parent skill (migrate-etl-package) for next steps.

## Troubleshooting

### `dbt seed` fails with column type mismatch
- Check seed CSV headers match the column names in `migrate_etl_package_test_sources.yml`
- Verify data types: dbt infers from CSV values — use consistent formats (dates as `YYYY-MM-DD`, numbers without commas)
- If a column should be `VARIANT` or `TIMESTAMP_NTZ`, add explicit column types in `migrate_etl_package_test_schema.yml`
...
- Check that `migrate_etl_package_test_sources.yml` points to the correct database/schema
...
- Seeds: `PACKAGE/.migrate-etl-package/tests/dbt/<project_name>/seeds/`
- Singular tests: `PACKAGE/.migrate-etl-package/tests/dbt/<project_name>/tests/`
- Schema test config: `migrate_etl_package_test_schema.yml`
- Source override config: `migrate_etl_package_test_sources.yml`
- Test report: `PACKAGE/.migrate-etl-package/tests/dbt/<project_name>/test_report.md`
...
- Write all files to `{PACKAGE}/.migrate-etl-package/tests/dbt/{PROJECT_NAME}/`
