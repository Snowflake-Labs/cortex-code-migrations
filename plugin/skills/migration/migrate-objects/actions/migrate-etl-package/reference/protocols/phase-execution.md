...
> **Auto-compact resilience:** Auto-compact (context summarization) may occur at any time during execution. Every phase type starts with a **Preamble** that re-reads canonical state from disk. Wave checkpoints between agent waves verify artifacts and tracking persisted. The orchestrator never trusts conversation context for precise state — it verifies from disk.

---

## Full TDD Phase

**Progress tracking:** Create a task at phase start and mark each step done immediately upon completion:
```bash
cortex ctx task add "Phase {N}: {phase_name}"
cortex ctx task start <task_id>
cortex ctx step add "a. Setup: create schemas + deploy infra" -t <task_id>
cortex ctx step add "b. Spawn test-gen agents" -t <task_id>
cortex ctx step add "c. Wait for test-gen + update tracking" -t <task_id>
cortex ctx step add "d. Spawn fix agents" -t <task_id>
cortex ctx step add "e. Wait for fix agents + update tracking" -t <task_id>
cortex ctx step add "f. Apply fixes (spawn apply-fixes agent)" -t <task_id>
cortex ctx step add "g. Post-apply validation" -t <task_id>
cortex ctx step add "h. Merge learnings" -t <task_id>
cortex ctx step add "i. Update tracking + complete phase" -t <task_id>
cortex ctx step add "j. Checkpoint" -t <task_id>
```
Mark the task done after step j.
...
**Progress tracking:**
```bash
cortex ctx task add "Phase {N}: {phase_name} (lightweight)"
cortex ctx task start <task_id>
cortex ctx step add "a. Setup: create schemas + deploy infra" -t <task_id>
cortex ctx step add "b. Spawn fix agents (apply known patterns)" -t <task_id>
cortex ctx step add "c-e. Wait for fix agents + update tracking" -t <task_id>
cortex ctx step add "f. Apply fixes (spawn apply-fixes agent)" -t <task_id>
cortex ctx step add "g. Post-apply validation" -t <task_id>
cortex ctx step add "h-j. Merge learnings + tracking + checkpoint" -t <task_id>
```
Mark the task done after step j.
...
### c-j. Same as Full TDD
Steps c through j follow the same protocol as Full TDD (wait for fix, apply fixes, post-apply validation, merge learnings, update tracking, checkpoint).
...
**Progress tracking:**
```bash
cortex ctx task add "Phase {N}: {phase_name} (dbt)"
cortex ctx task start <task_id>
cortex ctx step add "a. Setup: register nodes" -t <task_id>
cortex ctx step add "b. Spawn dbt-test-gen agents" -t <task_id>
cortex ctx step add "c. Wait for dbt-test-gen + validate artifacts" -t <task_id>
cortex ctx step add "d. Spawn dbt-fix agents" -t <task_id>
cortex ctx step add "e. Wait for dbt-fix + update tracking" -t <task_id>
cortex ctx step add "f. Merge dbt learnings" -t <task_id>
cortex ctx step add "g. Update tracking + complete phase" -t <task_id>
```
Mark the task done after step g.
...
- `{PACKAGE}/.migrate-etl-package/tests/dbt/{PROJECT_NAME}/seeds/` — at least 1 .csv file
- `{PACKAGE}/.migrate-etl-package/tests/dbt/{PROJECT_NAME}/tests/` — at least 1 .sql file
- `{PACKAGE}/.migrate-etl-package/tests/dbt/{PROJECT_NAME}/test_report.md`
...
1. For each project: glob `.migrate-etl-package/tests/dbt/{PROJECT_NAME}/seeds/*.csv`, `.migrate-etl-package/tests/dbt/{PROJECT_NAME}/tests/*.sql`, and check `.migrate-etl-package/tests/dbt/{PROJECT_NAME}/test_report.md` exists
...
**Progress tracking:**
```bash
cortex ctx task add "Phase {N}: Final Validation"
cortex ctx task start <task_id>
cortex ctx step add "1. Element status check" -t <task_id>
cortex ctx step add "2. EWI marker check" -t <task_id>
cortex ctx step add "3. Fix log consistency" -t <task_id>
cortex ctx step add "4. Artifact completeness" -t <task_id>
cortex ctx step add "5. Generate HTML report" -t <task_id>
```
Mark the task done after step 5.
...
```bash
uv run --project {SKILL_DIR} python {SKILL_DIR}/scripts/generate_report.py {PACKAGE_FOLDER}
```

Output: `{PACKAGE}/.migrate-etl-package/artifacts/report.html`

The report aggregates data from `scan_results.json`, `session_status.json`, `ROADMAP.md`, `PACKAGE_ORCH_CONTEXT.md`, `PACKAGE_DBT_CONTEXT.md`, `fix_log.md`, and `STATE.md` into a single HTML file with executive summary, phase-by-phase status, per-element details, dbt project status, fix log, and full artifact contents.
