...
### scan_package.py fails
- Verify package folder contains exactly one `.sql` orchestration file
...
- Verify `scan_results.json` exists in `{PACKAGE}/.migrate-etl-package/artifacts/planning/`
- Verify JSON is well-formed: `python -m json.tool {PACKAGE}/.migrate-etl-package/artifacts/planning/scan_results.json`
