...
Analyze the dbt sub-projects in a converted ETL package and produce a structural understanding document for dbt phase planning. Do NOT read {SKILL_DIR}/SKILL.md — all instructions are below.

Your inputs:
- Scan results: {PACKAGE}/.migrate-etl-package/artifacts/planning/scan_results.json (contains dbt_projects with health signals)
...
Write your output to: {PACKAGE}/.migrate-etl-package/artifacts/planning/PACKAGE_DBT_CONTEXT.md

**Cross-reference:** `PACKAGE_ORCH_CONTEXT.md` may exist in the same directory. If present, check its Element Inventory for elements with `EXECUTE DBT PROJECT` references — these are the orchestration-side links to dbt projects and provide context for how dbt models are invoked.
...
Do NOT ask the user any questions — make reasonable defaults and document assumptions in PACKAGE_DBT_CONTEXT.md.
