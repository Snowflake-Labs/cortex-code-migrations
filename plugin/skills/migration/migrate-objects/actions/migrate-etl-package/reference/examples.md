...
3. Use the `task` tool to spawn `name="context-mapper"` with `run_in_background=true` (no team during planning) → wait for task notification → verify `PACKAGE_ORCH_CONTEXT.md` and `PACKAGE_DBT_CONTEXT.md`
...
3. Use `task` tool to spawn dbt-test-gen agents for both **in a single message** (parallel):
   - name="dbt-test-gen-projectA", team_name="etl-fix-SalesLoad-p3p4", run_in_background=true
   - name="dbt-test-gen-projectB", team_name="etl-fix-SalesLoad-p3p4", run_in_background=true
...
5. Use `task` tool to spawn dbt-fixer agents for both **in a single message** (parallel):
   - name="dbt-fixer-projectA", team_name="etl-fix-SalesLoad-p3p4", run_in_background=true
   - name="dbt-fixer-projectB", team_name="etl-fix-SalesLoad-p3p4", run_in_background=true
...
3. Remaining count = 2 → use `task` tool to spawn continuation agent: name="orchestration-test-gen-B3.2-r1", team_name="etl-fix-PackageName-p3", run_in_background=true, with `{ELEMENT_LIST}` = `element_C, element_D` only (not the full original 4-element list)
