...
3. Use the `task` tool to spawn `name="context-mapper"` with `run_in_background=false` (no team during planning) → wait for task notification → verify `PACKAGE_ORCH_CONTEXT.md` and `PACKAGE_DBT_CONTEXT.md`
