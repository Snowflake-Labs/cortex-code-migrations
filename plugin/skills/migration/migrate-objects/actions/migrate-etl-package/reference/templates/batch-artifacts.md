...
Each orchestration batch agent writes a structured artifact at `{PACKAGE}/.migrate-etl-package/artifacts/phases/phase_{N}/batch_{B}.md`. This is the **only** output channel for batch results — the orchestrator reads these artifacts to update tracking and apply fixes.
...
- **Test file**: `{PACKAGE}/.migrate-etl-package/tests/orchestration/{task_procedure_name}/{element_name}.sql`
...
Each batch agent writes a per-batch learning file at `{PACKAGE}/.migrate-etl-package/artifacts/phases/phase_{N}/learnings_batch_{B}.md`. These are merged into `{PACKAGE}/.migrate-etl-package/artifacts/tracking/fix_log.md` at phase end by the orchestrator (single-writer).
