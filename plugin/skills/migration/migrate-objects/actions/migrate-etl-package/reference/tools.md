...
### `scan_package.py` — Package Scanner

Scans a converted ETL package folder and produces a structural JSON index.

```bash
uv run --project <SKILL_DIR> python <SKILL_DIR>/scripts/scan_package.py <package_folder> [<source_file>] [--platform <platform_id>]
```

- **Input**: Package folder path, optional source definition file path, optional `--platform` flag (auto-detected from extension if omitted)
- **Output**: `<package_folder>/.migrate-etl-package/artifacts/planning/scan_results.json`
- **Used in**: Planning Step 2

---

### `strip_dead_code.py` — Dead Code Stripper (SSIS)

Removes commented-out C#/XML boilerplate and base64 binary data from SSIS0004 ScriptTask blocks, keeping only ScriptMain.cs business logic. Located at `platforms/ssis/strip_dead_code.py`.

```bash
uv run --project <SKILL_DIR> python <SKILL_DIR>/platforms/ssis/strip_dead_code.py <orchestration_sql_file>
```
...
- **Note**: Other platforms may have their own strip scripts, or none (if `strip_script: null` in the platform profile, this step is skipped)
...
| Command | Usage |
|---------|-------|
| `init` | `track_status.py init <scan_results.json>` — Initialize session from scan results |
...
Generates a self-contained HTML report from all ETL fixer artifacts. Aggregates `scan_results.json`, `session_status.json`, `ROADMAP.md`, `PACKAGE_ORCH_CONTEXT.md`, `PACKAGE_DBT_CONTEXT.md`, `fix_log.md`, and `STATE.md` into a single browsable HTML file.

```bash
uv run --project <SKILL_DIR> python <SKILL_DIR>/scripts/generate_report.py <package_folder> [--template <path>]
```

- **Input**: Package folder path; optional `--template` for custom HTML template (defaults to `reference/templates/report-template.html`)
- **Output**: `<package_folder>/.migrate-etl-package/artifacts/report.html`
