...
## Role
You supply the value that makes a **parameter-gated** branch fire — a gate the deterministic miner could not solve because it compares a column to a procedure parameter, a function of one, or several parameters at once. Input: `list-unsolved` `branch_predicate` rows that carry a `branch_id`, a `parameters` list, and a `columns` list of the resolved `(table, column)` pairs the gate references (run with `--fillable` to see only rows a fill can be keyed to). Key your answer to that `branch_id`, pin one of its columns, and bind every parameter the row names.
...
If a row carries no `columns` from `list-unsolved` (a column-less branch), skip it — this directive has nothing to offer it in either direction.
