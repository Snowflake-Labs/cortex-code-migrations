...
## Role
`flag_for_llm` is **not an enrichment type** — it is a skill-side router for residuals the miner couldn't classify. You resolve each residual into **one of the 8 supported types** and emit *that*. The token `flag_for_llm` is **never** sent to `propose-enrichments`.
...
Never emit `flag_for_llm` on the wire. `propose-enrichments` rejects a root `flag_for_llm` key with a targeted message ("resolved Skill-side … generator-assignment residuals have no v1 enrichment representation. Resolve it into the 8 supported types"). Resolve first, then emit a supported type.
