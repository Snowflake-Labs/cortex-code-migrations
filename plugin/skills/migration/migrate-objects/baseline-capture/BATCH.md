...
> Called from [../SKILL.md](../SKILL.md) Step 2 when the user opts for batch test case generation.
...
```
query_registry(
  where="source.objectType IN ('function', 'procedure') AND source.name IN ('<name1>', '<name2>', ...)",
...
Return control to the parent skill ([../SKILL.md](../SKILL.md) Step 3: Dispatch Loop).
