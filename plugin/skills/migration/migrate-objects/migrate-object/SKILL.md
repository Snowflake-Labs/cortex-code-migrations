...
scai test validate -c <CONNECTION_NAME> \
  --where "source.canonicalName ILIKE '%<schema>.<name>%'"
```
...
|-----------------------|--------|
| Not in registry / not deployed | **Skip this object.** Report: "Blocked on `<dependency>` — not yet deployed. Moving to next object." Return to [../SKILL.md](../SKILL.md) Step 4 to pick a different object. |
| Deployed but tests not passing | **Skip this object.** Report: "Blocked on `<dependency>` — deployed but not yet passing tests. Fix that object first." Return to [../SKILL.md](../SKILL.md) Step 4. |
...
5. **Return to caller** for next object — go back to [../SKILL.md](../SKILL.md) Step 4 (Dispatch Loop).
