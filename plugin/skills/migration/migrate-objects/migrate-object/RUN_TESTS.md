...
| `nextTask: null`, not errored, not blocked | The dependency is done. If the FAIL is still a defect **in that code unit** (wrong column type, bad view body, missing CAST on *its* SQL), do **not** patch this procedure around it. **Under `subagent_mode`:** spawn one foreground [`task_invalidator`](../../../../agents/task_invalidator.md) (`subagent_type="task_invalidator"`) with that code unit's `id` and why — plus this waiter's `objectId` so its verification is also reopened. Omit the resume task; the server picks the verification node. Then **return** `result: "reopened"` with `reopenedCodeUnits`. Do not stay in the fix loop. **Interactive session:** tell the user which dependency needs a fix and wait — do not spawn `task_invalidator`, do not return walker JSON. If the dependency's SQL is faithful and the FAIL is in *this* code unit, this is not a dependency failure — proceed to fix the code. |
...
narrower temp column, a filter the fixture never produced):

- **Under `subagent_mode`:** spawn
  ...
  yaml is the binding.
- **Interactive session:** tell the user the fixture cannot exercise the
  object, and wait. Do not spawn `sandbox_specialist`. Do not stamp
  `error=sql`.
