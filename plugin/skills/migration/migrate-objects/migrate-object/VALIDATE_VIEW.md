...
| Fixture rows make the view untestable (empty join, filter the testbed never produced) | Source and Snowflake agree on empty/wrong, and a different legal dataset would exercise the view | *(do not stamp — spawn `sandbox_specialist` with `task=validateView`)* |
