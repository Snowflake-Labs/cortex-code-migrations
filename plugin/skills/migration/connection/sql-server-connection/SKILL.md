...
```bash
scai connection test -l sqlserver -c <CONNECTION_NAME>
```
...
| Add connection (inline) | `scai connection add-sql-server -c NAME --auth standard --server-url HOST --database DB --user USER` |
| Test connection | `scai connection test -l sqlserver -c NAME` |
| Set default | `scai connection set-default -l sqlserver -c NAME` |
