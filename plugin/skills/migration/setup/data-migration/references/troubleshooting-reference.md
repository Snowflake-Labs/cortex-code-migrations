...
**Symptom:** You submitted a workflow via `scai data cloud-migrate`, but the orchestrator service logs (`CALL SYSTEM$GET_SERVICE_LOGS(...)`) repeatedly show `Found 0 pending workflows`.
...
**Symptom:** `scai data setup-cloud-migration` or `scai data cloud-migrate` fails with privilege errors like "current role has no privileges" on `DATA_MIGRATION_SERVICE`.
...
**Symptom:** `scai data cloud-migrate --start-service` appears to succeed, but `SELECT SYSTEM$GET_SERVICE_STATUS(...)` returns `[]` (no running instances).
...
-- Stop/suspend the service when done
-- ALTER SERVICE SNOWCONVERT_AI.DATA_MIGRATION.DATA_MIGRATION_SERVICE SUSPEND;
```
