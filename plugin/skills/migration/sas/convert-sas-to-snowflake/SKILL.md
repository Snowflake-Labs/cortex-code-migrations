...
**Load `references/conversion-rules.md` at Steps 5-6** (code generation and self-check). Contains 17 rules governing semantic correctness.
...
| 5 | `conversion-rules.md` + construct-specific refs (data-steps, proc-sql, etc.) |
...
15. **Validation reference tables match SAS** — for each anti-join validation check, the reference table in the converted SQL must match the reference table in the SAS source; a master table (e.g., `ORGANIZATION`) must not be replaced with a derived/subset table (e.g., `ORG_UNIT_DIMENSION`)
