...
INSERT INTO fluid_interval VALUES ('W1','Ile Fm', 1375.0, 1420.0);  -- [top, base] in METERS
-- Child measurement table — MD values chosen to overlap the interval
INSERT INTO mineralogy VALUES ('W1', 1380.0, 55.2);  -- inside  (positive)
INSERT INTO mineralogy VALUES ('W1', 1410.0, 48.5);  -- inside  (positive)
INSERT INTO mineralogy VALUES ('W1', 1375.0, 50.0);  -- top boundary
INSERT INTO mineralogy VALUES ('W1', 1600.0, 40.0);  -- outside (negative test)
```
...
**Worked example (real failure):** a pipeline joined `MINERALOGY_TO_OW.DEPTH_M` (meters) to `BRAIN_FLUID_CPLIT.TOP/BASE_PICK_DEPTH_MD`. The interval depths were seeded in feet (4500–5200) while the measurements were in meters (1175–1540). Every `BETWEEN` join returned zero rows, so all downstream tables were empty — yet every file "passed" compilation and isolated execution.

Rule:
- Identify quantity families by name stem and unit suffix: `MD`, `MD_FT`, `MD_M`, `MD_MRKB`, `DEPTH`, `DEPTH_M`, `TOP_DEPTH`, `TOP_PICK_DEPTH_MD`, `BASE_*`, etc.
...
INSERT INTO well_logs (WELLBORE_NAME, MD_FT, MD_M) VALUES ('W1', 1380.0, 1380.0); -- both cols same range
INSERT INTO wireline_test_data (COP_UWI, MD_MRKB) VALUES ('W1', 1390.0);
INSERT INTO dst_data (COP_UWI, MD_M) VALUES ('W1', 1395.0);
```
...
Verification heuristic: pick one well/entity (e.g. `W1`) and ensure it has aligned rows in EVERY source table on the golden path, so it flows through every layer to the terminal tables.
