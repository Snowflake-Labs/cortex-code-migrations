...
- Common pattern: SAS creates a small lookup/filter table (e.g., `HOSP_LIST` with specific MEDCR_IDs), then uses those IDs in a WHERE clause like `WHERE T1.MEDCR_ID IN (&MEDCR_ID.)`. The converted SQL must preserve BOTH the filter dataset creation AND its application as a WHERE/JOIN filter.
...
- When SAS validates values against a dimension/lookup table (e.g., `IF Activity_Type NOT IN (SELECT ... FROM dim_table)`), the converted SQL **MUST** preserve the table-based lookup
- **NEVER** replace a SAS dimension-table lookup with a hardcoded value list (e.g., `NOT IN ('VR','NVR','BOTH')`)
...
- When SAS validation checks anti-join against a **master/reference table** (e.g., `Inform.ORGANIZATION`, `Inform.COST_DRIVERS`, `Inform.PRODUCT`), the converted SQL **MUST** anti-join against the **same semantic equivalent** — not a derived, subset, or workpaper-local table
- A master hierarchy table (e.g., `ORGANIZATION`) and a workpaper-derived table (e.g., `ORG_UNIT_DIMENSION`) are **NOT interchangeable** — the master is the authoritative source; the workpaper is a user-entered subset
...
- If SAS validates Activity_ID against tables A, B, and C, the converted SQL must also validate against A, B, and C — not just A
...
- Consolidation **MUST NOT change which reference table** a validation check validates against — if SAS check N anti-joins against `Inform.ORGANIZATION`, the consolidated SQL must also anti-join against `ORGANIZATION` (or its schema-qualified equivalent), not a derived table like `ORG_UNIT_DIMENSION`
