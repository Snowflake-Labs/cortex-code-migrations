...
| SnowConvert dialect registry | SAS is not a SnowConvert source dialect in v1 |

Adding SAS to AIM claims, waves, `OBJECT_CLAIMS`, or the migration `<skill-match>` would require a separate product design — not this parallel track.
...
- Router + three child `SKILL.md`s + validate sub-skill exist
