...
Set under any `[connections.source.<engine>]` section to apply source-wide SQL hints (merged with workflow-level `queryModifiers`):
...
| Key | Description |
|-----|-------------|
| `objectModifier` | Hint on the table in `FROM` |
| `selectModifier` | Hint after `SELECT` |
